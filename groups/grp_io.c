...
static t_promptret	grp_getio_applytoken(t_grp *grp,
	t_token *token, size_t *pos)
{
	t_bool		found;
	t_promptret	status;
	...
	while (grp->input_after_uniq[*pos])
	{
		if (grp_canapply(grp->reader)
			&& (ft_isspace(grp->input_after_uniq[*pos])
				|| tok_iscontrole(grp->input_after_uniq[*pos])) && found)
			break ;
		else if (grp_canapply(grp->reader)
			&& (ft_isspace(grp->input_after_uniq[*pos])
				|| tok_iscontrole(grp->input_after_uniq[*pos])))
			(void)found;
		else
		{
			status = grp_read_addchar(grp->reader,
					grp->input_after_uniq[*pos], TRUE);
...
static t_promptret	grp_getio_ontoken(t_grp *grp, size_t *pos)
{
	...
	if (!tok_is_redir(ntoken))
	{
		tok_clear(&ntoken);
		*pos = start_pos;
		return (grp_read_addchar(grp->reader,
				grp->input_after_uniq[*pos], FALSE));
	}
	...
	(*pos)--;
	return (grp_getio_ontoken_ret(&ntoken, status));
}

...
/*   Updated: 2025/04/08 13:11:11 by kgauthie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "groups.h"

static void	grp_check_uniq_loop_init(t_bool *set_started, size_t *pos)
{
	*pos = 0;
	*set_started = FALSE;
}
...
static t_promptret	grp_check_uniq_loop(t_grp *grp)
{
	...
	grp_check_uniq_loop_init(&set_started, &pos);
	while (grp->input_after_uniq[pos])
	{
		set = FALSE;
		status = grp_read_addchar_at(grp->reader,
				grp->input_after_uniq[pos], 1, &set);

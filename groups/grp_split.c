...
/*   Updated: 2025/04/06 13:46:30 by kgauthie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_promptret	grp_set_split_ontoken(t_grp *grp,
	t_token_type target_type, size_t *pos)
{
	t_token	*ntoken;
	size_t	start_pos;
...
t_promptret	grp_set_split(t_grp *grp, char token, t_token_type target_type)
{
	...
	while (grp->input[pos])
	{
		if (grp->input[pos] == token
			&& grp_canapply(grp->reader) && !grp->token)
		{
